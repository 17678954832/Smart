...
public class UserServiceImpl implements IUserService {
     ...
     */
    @Override
    public ResponseView<User> register(User user) {

        //校验用户名、邮箱不能重复
        int countByUsername = userMapper.countByUsername(user.getUsername());
        if (countByUsername > 0){
            return ResponseView.error(ResponseEnum.USERNAME_EXIST);
        }
        int countByEmail = userMapper.countByEmail(user.getEmail());
        if (countByEmail > 0){
            return ResponseView.error(ResponseEnum.EMAIL_EXIST);
        }
    ...
    @Override
    public ResponseView<User> login(String username, String password) {
        User user = userMapper.selectByUsername(username);
        //判断用户是否存在
        if (user == null){
            //用户不存在(返回用户名或密码错误)
            return ResponseView.error(ResponseEnum.USERNAME_OR_PASSWORD_ERROR);
        }
